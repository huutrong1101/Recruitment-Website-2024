export const data = {
  listType: ['Part Time', 'Full Time', 'Remote'],
  jobs: [
    {
      jobId: '1',
      name: 'Job 1',
      jobType: 'Type 1',
      quantity: 5,
      benefit: 'Benefit 1',
      salaryRange: 'Range 1',
      requirement: 'Requirement 1',
      location: 'Location 1',
      description: 'Description 1',
      isActive: true,
      createdAt: '2023-08-21',
      updatedAt: '2023-08-21',
      deadline: '2023-08-31',
      position: {
        positionId: '1',
        name: 'Position 1'
      },
      skills: [
        {
          skillId: '1',
          name: 'Skill 1'
        },
        {
          skillId: '2',
          name: 'Skill 2'
        }
      ]
    },
    {
      jobId: '2',
      name: 'Job 2',
      jobType: 'Type 2',
      quantity: 3,
      benefit: 'Benefit 2',
      salaryRange: 'Range 2',
      requirement: 'Requirement 2',
      location: 'Location 2',
      description: 'Description 2',
      isActive: true,
      createdAt: '2023-08-21',
      updatedAt: '2023-08-21',
      deadline: '2023-08-31',
      position: {
        positionId: '2',
        name: 'Position 2'
      },
      skills: [
        {
          skillId: '3',
          name: 'Skill 3'
        },
        {
          skillId: '4',
          name: 'Skill 4'
        }
      ]
    },
    {
      jobId: '3',
      name: 'Job 3',
      jobType: 'Type 3',
      quantity: 3,
      benefit: 'Benefit 3',
      salaryRange: 'Range 3',
      requirement: 'Requirement 3',
      location: 'Location 3',
      description: 'Description 3',
      isActive: true,
      createdAt: '2023-08-21',
      updatedAt: '2023-08-21',
      deadline: '2023-08-31',
      position: {
        positionId: '2',
        name: 'Position 2'
      },
      skills: [
        {
          skillId: '3',
          name: 'Skill 3'
        },
        {
          skillId: '4',
          name: 'Skill 4'
        }
      ]
    },
    {
      jobId: '4',
      name: 'Job 4',
      jobType: 'Type 3',
      quantity: 3,
      benefit: 'Benefit 3',
      salaryRange: 'Range 3',
      requirement: 'Requirement 3',
      location: 'Location 3',
      description: 'Description 3',
      isActive: true,
      createdAt: '2023-08-21',
      updatedAt: '2023-08-21',
      deadline: '2023-08-31',
      position: {
        positionId: '2',
        name: 'Position 2'
      },
      skills: [
        {
          skillId: '3',
          name: 'Skill 3'
        },
        {
          skillId: '4',
          name: 'Skill 4'
        }
      ]
    },
    {
      jobId: '5',
      name: 'Job 5',
      jobType: 'Type 3',
      quantity: 3,
      benefit: 'Benefit 3',
      salaryRange: 'Range 3',
      requirement: 'Requirement 3',
      location: 'Location 3',
      description: 'Description 3',
      isActive: true,
      createdAt: '2023-08-21',
      updatedAt: '2023-08-21',
      deadline: '2023-08-31',
      position: {
        positionId: '2',
        name: 'Position 2'
      },
      skills: [
        {
          skillId: '3',
          name: 'Skill 3'
        },
        {
          skillId: '4',
          name: 'Skill 4'
        }
      ]
    },
    {
      jobId: '6',
      name: 'Job 6',
      jobType: 'Type 3',
      quantity: 3,
      benefit: 'Benefit 3',
      salaryRange: 'Range 3',
      requirement: 'Requirement 3',
      location: 'Location 3',
      description: 'Description 3',
      isActive: true,
      createdAt: '2023-08-21',
      updatedAt: '2023-08-21',
      deadline: '2023-08-31',
      position: {
        positionId: '2',
        name: 'Position 2'
      },
      skills: [
        {
          skillId: '3',
          name: 'Skill 3'
        },
        {
          skillId: '4',
          name: 'Skill 4'
        }
      ]
    }
  ],
  events: [
    {
      id: '1',
      title: 'Event 1',
      name: 'Event Name 1',
      description: 'Event Description 1',
      img: 'https://images.unsplash.com/photo-1591269469224-0479a5f956c6?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1170&q=80',
      author: 'Author 1',
      linkContacts: {
        Instagram: 'https://www.instagram.com/event1',
        LinkedIn: 'https://www.linkedin.com/event1',
        Twitter: 'https://www.twitter.com/event1',
        Facebook: 'https://www.facebook.com/event1',
        Gitlab: 'https://www.gitlab.com/event1'
      },
      startAt: '2023-08-21',
      deadline: '2023-08-31',
      time: '10:00 AM',
      location: 'Location 1'
    },
    {
      id: '2',
      title: 'Event 2',
      name: 'Event Name 2',
      description: 'Event Description 2',
      img: 'https://images.unsplash.com/photo-1591269469224-0479a5f956c6?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1170&q=80',
      author: 'Author 1',
      linkContacts: {
        Instagram: 'https://www.instagram.com/event1',
        LinkedIn: 'https://www.linkedin.com/event1',
        Twitter: 'https://www.twitter.com/event1',
        Facebook: 'https://www.facebook.com/event1',
        Gitlab: 'https://www.gitlab.com/event1'
      },
      startAt: '2023-08-21',
      deadline: '2023-08-31',
      time: '10:00 AM',
      location: 'Location 1'
    },
    {
      id: '3',
      title: 'Event 3',
      name: 'Event Name 3',
      description: 'Event Description 3',
      img: 'https://images.unsplash.com/photo-1591269469224-0479a5f956c6?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1170&q=80',
      author: 'Author 1',
      linkContacts: {
        Instagram: 'https://www.instagram.com/event1',
        LinkedIn: 'https://www.linkedin.com/event1',
        Twitter: 'https://www.twitter.com/event1',
        Facebook: 'https://www.facebook.com/event1',
        Gitlab: 'https://www.gitlab.com/event1'
      },
      startAt: '2023-08-21',
      deadline: '2023-08-31',
      time: '10:00 AM',
      location: 'Location 1'
    }
  ],
  posistion: ['FRONTEND', 'BACKEND', 'FULLSTACK'],
  location: ['F-Town1', 'F-town2', 'F-town3'],
  showJobLists: [
    {
      userId: '02aed1c4-ce1f-4638-9a49-b06ee33ea7ab',
      phone: '0777752399',
      email: 'nguyennh@gmail.com',
      fullName: 'Nguyễn Huỳnh Nguyên',
      avatar:
        'data:image/jpeg;base64,',
      address: 'Thủ Đức-TPHCM',
      dateOfBirth: null,
      about: 'Null',
      gender: 'MALE',
      createdAt: '2023-07-20T03:24:54.849+07:00',
      updatedAt: '2023-08-01T15:08:39.052+07:00',
      role: 'CANDIDATE',
      blacklist: true,
      active: true,
      information: {
        skills: [
          {
            label: 'C++'
          },
          {
            label: 'JavaScript'
          }
        ]
      }
    },
    {
      userId: '03eb1bf2-d83d-4e72-b9c1-2778e6712edd',
      phone: '0123123123',
      email: 'toannguyen7202@gmail.com',
      fullName: 'Nguyễn Hoàng Toàn',
      avatar: null,
      address: null,
      dateOfBirth: null,
      about: null,
      gender: null,
      createdAt: '2023-08-01T10:42:45.636+07:00',
      updatedAt: null,
      role: 'CANDIDATE',
      information: {
        skills: [
          {
            label: 'C++'
          },
          {
            label: 'JavaScript'
          }
        ]
      },
      blacklist: true,
      active: true
    },
    {
      userId: '13b90afc-acde-4a82-a679-2537ffb6a609',
      phone: '012334567890247',
      email: 'nhatlt@gmail.com',
      fullName: 'Lê Nhật Tiến',
      avatar:
        'data:image/jpeg;base64,',
      address: 'Tp Hồ Chí Minh',
      dateOfBirth: null,
      about: 'Null',
      gender: 'FEMALE',
      createdAt: '2023-07-17T16:36:03.114+07:00',
      updatedAt: '2023-08-03T13:22:43.666+07:00',
      role: 'CANDIDATE',
      information: {
        skills: [
          {
            label: 'C++'
          },
          {
            label: 'JavaScript'
          }
        ]
      },
      blacklist: true,
      active: true
    },
    {
      userId: '232bf658-e457-46d7-a257-d89ac299609c',
      phone: '0373417457',
      email: 'annv123@gmail.com',
      fullName: 'Nguyễn Văn An',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690452509/Recruiment%20Assets/User/p8x0pulbulhjkfodk37y.jpg',
      address: 'Thủ Đức-TPHCM',
      dateOfBirth: null,
      about: 'Null',
      gender: 'FEMALE',
      createdAt: '2023-07-20T14:29:34.489+07:00',
      updatedAt: '2023-08-02T19:41:19.943+07:00',
      role: 'CANDIDATE',
      information: {
        skills: [
          {
            label: 'C++'
          },
          {
            label: 'JavaScript'
          }
        ]
      },
      blacklist: false,
      active: true
    },
    {
      userId: '242208ac-fb2c-4c48-ad57-dc93fa03d05f',
      phone: '0123345678936.2152083068489',
      email: 'sonlm7227@gmail.com',
      fullName: 'Lê Minh Sơn',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690452509/Recruiment%20Assets/User/p8x0pulbulhjkfodk37y.jpg',
      address: 'Tp Hồ Chí Minh',
      dateOfBirth: null,
      about: 'Null',
      gender: 'MALE',
      createdAt: '2023-07-17T16:28:42.354+07:00',
      updatedAt: null,
      role: 'CANDIDATE',
      information: {
        skills: [
          {
            label: 'C++'
          },
          {
            label: 'JavaScript'
          }
        ]
      },
      blacklist: false,
      active: true
    },
    {
      userId: '2466f1a9-f799-426f-b33d-670daa7c7a54',
      phone: '01231231231231231',
      email: 'daudaua3782@gmail.com',
      fullName: 'Nguyễn Huỳnh Nguyên',
      avatar: null,
      address: null,
      dateOfBirth: null,
      about: null,
      gender: null,
      createdAt: '2023-08-04T10:28:23.985+07:00',
      updatedAt: '2023-08-04T10:59:29.371+07:00',
      role: 'CANDIDATE',
      information: {
        skills: [
          {
            label: 'C++'
          },
          {
            label: 'JavaScript'
          }
        ]
      },
      blacklist: false,
      active: true
    },
    {
      userId: '43d97677-8c75-43e9-9514-12940f5e6f39',
      phone: '0123345678974.32105674776632',
      email: 'admin4587@gmail.com',
      fullName: 'Admin Is Me',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690452509/Recruiment%20Assets/User/p8x0pulbulhjkfodk37y.jpg',
      address: 'Tp Hồ Chí Minh',
      dateOfBirth: null,
      about: 'Null',
      gender: 'FEMALE',
      createdAt: '2023-07-17T14:42:55.971+07:00',
      updatedAt: '2023-08-04T10:00:14.190+07:00',
      role: 'CANDIDATE',
      information: {
        skills: [
          {
            label: 'C++'
          },
          {
            label: 'JavaScript'
          }
        ]
      },
      blacklist: false,
      active: true
    },
    {
      userId: '4be32c6c-cd0e-4a0f-8cf8-91189cb27f6e',
      phone: '4444444444444444',
      email: 'phuc.nguyen0902@hcmut.edu.vn',
      fullName: 'Higo Nguyễn',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690986684/Recruiment%20Assets/User/rbxacambe0hqmvfmxm0s.jpg',
      address: null,
      dateOfBirth: null,
      about: null,
      gender: null,
      createdAt: '2023-08-02T21:29:42.372+07:00',
      updatedAt: '2023-08-03T15:26:12.694+07:00',
      role: 'CANDIDATE',
      information: {
        skills: [
          {
            label: 'C++'
          },
          {
            label: 'JavaScript'
          }
        ]
      },
      blacklist: false,
      active: true
    },
    {
      userId: '51512f58-0f0f-4fea-a76f-44f017ba00a0',
      phone: '0942594519',
      email: 'lethanhkhai12@gmail.com',
      fullName: 'Khải nè',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1691084247/Recruiment%20Assets/User/rme2f3mn4ofaoky83pcj.png',
      address: null,
      dateOfBirth: null,
      about: null,
      gender: null,
      createdAt: '2023-08-03T10:42:40.318+07:00',
      updatedAt: '2023-08-04T02:25:31.116+07:00',
      role: 'CANDIDATE',
      information: {
        skills: [
          {
            label: 'C++'
          },
          {
            label: 'JavaScript'
          }
        ]
      },
      blacklist: false,
      active: true
    },
    {
      userId: '55f82846-6d79-4b1d-8e53-f0d3b50dd341',
      phone: '123456789',
      email: 'olfjoker@gmail.com',
      fullName: 'ncp',
      avatar: null,
      address: null,
      dateOfBirth: null,
      about: null,
      gender: null,
      createdAt: '2023-08-01T16:14:18.862+07:00',
      updatedAt: null,
      role: 'CANDIDATE',
      information: {
        skills: [
          {
            label: 'C++'
          },
          {
            label: 'JavaScript'
          }
        ]
      },
      blacklist: false,
      active: true
    }
  ],
  candidate: [
    {
      userId: '4495048c-484c-42fa-bdce-e410d04c2c1a',
      email: 'thanh20080123kg@gmail.com',
      phone: '0394595118',
      fullName: 'An Ngo ne ne',
      avatar: null,
      address: null,
      about: null,
      skills: [
        {
          skillId: '1',
          name: 'Python'
        },
        {
          skillId: '15',
          name: 'AWS'
        },
        {
          skillId: '11',
          name: 'SQL '
        }
      ],
      dateOfBirth: null,
      jobIds: ['4ad65044-7134-4134-9a0f-483ad08d50b6'],
      information:
        '{"education":[],"experience":[{"companyName":"FPT","position":"BE","dateFrom":"2023-08-11","dateTo":"2023-08-27"}],"certificate":[],"project":[],"skills":[{"label":"Python","value":"1"},{"label":"AWS","value":"15"},{"label":"SQL ","value":"11"}]}',
      blackList: false
    },
    {
      userId: 'a6b25a1d-2dcf-41b2-95cb-975b059b1689',
      email: 'qquocchi1905@gmail.com',
      phone: '0362858058',
      fullName: 'Bùi Đặng Quốc Chí ',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1691377892/Recruiment%20Assets/User/x1cow3kctuykkqzxnugk.jpg',
      address: 'Thủ Đức',
      about:
        'Có khả năng diễn đạt ý kiến một cách rõ ràng và hiệu quả. Thông thao ngôn ngữ lập trình Java với 5 năm kinh nghiệm',
      skills: [
        {
          skillId: '2',
          name: 'Java'
        },
        {
          skillId: '4',
          name: 'C++'
        },
        {
          skillId: '5',
          name: 'JavaScript'
        },
        {
          skillId: '3',
          name: 'C#'
        },
        {
          skillId: '6',
          name: 'NodeJs'
        },
        {
          skillId: '7',
          name: 'ReactJs'
        }
      ],
      dateOfBirth: '2002-05-19T07:00:00.000+07:00',
      jobIds: [
        '7c2a98ff-2f13-4eab-b858-6d98bbb0bd6f',
        '85315764-31e9-4688-a83c-6f733122700b',
        '8a010f3b-19b2-4188-92de-e3c3373eeb30',
        'a61fc56e-0d6f-4de4-8a48-f6084831d341'
      ],
      information:
        '{"education":[{"school":"HCMUTE","major":"CNTT","graduatedYear":"2020-2024"}],"experience":[{"companyName":"HCMUTE","position":"Student","dateFrom":"2020-10-10","dateTo":"2024-10-07"}],"certificate":[{"name":"AWS  RDS","id":"123","receivedDate":"2023-08-10","url":"https://www.facebook.com/quoc.chi.752487/"}],"project":[],"skills":[{"label":"Java","value":"2"},{"label":"C++","value":"4"},{"label":"JavaScript","value":"5"},{"label":"C#","value":"3"},{"label":"NodeJs","value":"6"},{"label":"ReactJs","value":"7"}]}',
      blackList: false
    },
    {
      userId: '43a0413a-a2f3-4208-bf07-09d8bfc0609c',
      email: 'lethkhai@gmail.com',
      phone: '0303948284',
      fullName: 'Le KHai',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1691470272/Recruiment%20Assets/User/h7ibrqhnz15un2klcrri.jpg',
      address: 'thôn Quảng Thành II , xã Nghĩa Thành , huyện Châu Đức , tỉnh BRVT',
      about: '2+ Years with React',
      skills: [
        {
          skillId: '7',
          name: 'ReactJs'
        },
        {
          skillId: '5',
          name: 'JavaScript'
        }
      ],
      dateOfBirth: '2002-03-27T07:00:00.000+07:00',
      jobIds: ['73a45601-e894-431c-a512-2f46345441b5'],
      information:
        '{"education":[{"school":"International Unversity","major":"Bachelor Engineering of Information Technology","graduatedYear":"2024"}],"experience":[],"certificate":[],"project":[],"skills":[{"label":"ReactJs","value":"7"},{"label":"JavaScript","value":"5"}]}',
      blackList: false
    },
    {
      userId: '51512f58-0f0f-4fea-a76f-44f017ba00a0',
      email: 'lethanhkhai12@gmail.com',
      phone: '0942594519',
      fullName: 'Lê Thanh Khải',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1691084247/Recruiment%20Assets/User/rme2f3mn4ofaoky83pcj.png',
      address: 'Đà Lạt',
      about: 'Mong muốn tìm được công việc ổn định và phù hợp với định hướng tương lai',
      skills: [
        {
          skillId: '7',
          name: 'ReactJs'
        },
        {
          skillId: '2',
          name: 'Java'
        }
      ],
      dateOfBirth: '2002-04-12T00:00:00.000+07:00',
      jobIds: [
        '5f3365ec-d703-4b8f-8569-84f224e0625c',
        '73a45601-e894-431c-a512-2f46345441b5',
        '7ce8fd92-f96e-4b1f-99da-1c41625c6f31',
        '8a010f3b-19b2-4188-92de-e3c3373eeb30',
        'a61fc56e-0d6f-4de4-8a48-f6084831d341',
        'ff9fb400-15da-40a9-b8fc-991d1222f25a'
      ],
      information:
        '{"education":[{"school":"BK","major":"IT","graduatedYear":"2030"},{"school":"BK 2","major":"study again","graduatedYear":"2040"}],"experience":[{"companyName":"FPT","position":"Dev","dateFrom":"2023-06-12","dateTo":"2023-08-12"}],"certificate":[{"name":"FB user","id":"Khải","receivedDate":"2015-01-01","url":"https://www.facebook.com/khai.thanh.1204xx/"},{"name":"INS user","id":"id","receivedDate":"2020-01-01","url":""}],"project":[{"name":"Mock Project","description":"Recruitment","url":"http://localhost:5173/"},{"name":"Youtube","description":"Small Project","url":"https://www.youtube.com/watch?v=muAFXVFMUoA"}],"skills":[{"value":"reactjs","label":"ReactJS"},{"value":"java","label":"Java"},{"label":"VueJs","value":"8"},{"label":"C#","value":"3"}]}',
      blackList: false
    },
    {
      userId: '646110e3-2f07-4d44-b929-11fc9f6ae295',
      email: 'ganomol993@wiemei.com',
      phone: '0777752838',
      fullName: 'Nguyen Huynh Nguyen',
      avatar: null,
      address: null,
      about: null,
      skills: [],
      dateOfBirth: '1997-02-14T07:00:00.000+07:00',
      jobIds: ['73a45601-e894-431c-a512-2f46345441b5'],
      information:
        '{"education":[{"school":"IU","major":"Information  Technology","graduatedYear":"2024"}],"experience":[],"certificate":[],"project":[{"name":"Github Octocat","description":"AAAA","url":"http://github.com"}],"skills":[{"label":"Java","value":"2"},{"label":"ReactJs","value":"7"},{"label":"JavaScript","value":"5"}]}',
      blackList: false
    },
    {
      userId: '8ebab025-9664-4561-8d8d-7fe54ebf2c9f',
      email: 'xyvole@hexi.pics',
      phone: '092039283932',
      fullName: 'Nguyen Tuan Anh',
      avatar: null,
      address: null,
      about: null,
      skills: [],
      dateOfBirth: null,
      jobIds: ['73a45601-e894-431c-a512-2f46345441b5'],
      information: null,
      blackList: false
    },
    {
      userId: '684b3ac0-338d-11ee-be56-0242ac120002',
      email: 'candidate5@gmail.com',
      phone: '0369863254',
      fullName: 'Nguyễn Gia Khiêm',
      avatar: null,
      address: 'Khánh Hoà',
      about: 'Đam mê lập trình, đoạt nhiều giải trong các cuộc thi lớn',
      skills: [],
      dateOfBirth: '2001-09-17T20:33:18.000+07:00',
      jobIds: ['a61fc56e-0d6f-4de4-8a48-f6084831d341'],
      information:
        '{"education":[],"experience":[{"companyName":"FPT","position":"dev","dateFrom":"2023-01-02","dateTo":"2024-04-02"}],"certificate":[],"project":[{"name":"Git","description":"Mock","url":"https://gitlab.com/PlayerNguyen/recruiment-client"}],"skills":[{"label":"Java","value":"2"},{"label":"NodeJs","value":"6"},{"label":"C++","value":"4"},{"label":"VueJs","value":"8"},{"label":"JavaScript","value":"5"}]}',
      blackList: false
    },
    {
      userId: 'cc9cfe0c-0b43-4cca-8db4-f04936f74c82',
      email: 'nhnguyen.forwork@gmail.com',
      phone: '0777752703',
      fullName: 'Nguyễn Huỳnh Nguyên',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690861840/Recruiment%20Assets/User/d7n2n3sxpruvzyxmh2uy.jpg',
      address: 'Long Binh, Thu Duc City, Ho Chi Minh City',
      about: 'The Frontend Developer with 2+ years experiences.',
      skills: [],
      dateOfBirth: '2002-03-27T07:00:00.000+07:00',
      jobIds: [
        '2',
        '423a4b6c-dd3b-4710-a3f1-5e37496d1d64',
        '5f3365ec-d703-4b8f-8569-84f224e0625c',
        '73a45601-e894-431c-a512-2f46345441b5',
        '78e40a19-d592-4f89-b250-eb3008706711',
        '7c2a98ff-2f13-4eab-b858-6d98bbb0bd6f',
        '7ce8fd92-f96e-4b1f-99da-1c41625c6f31',
        '85315764-31e9-4688-a83c-6f733122700b',
        '8a010f3b-19b2-4188-92de-e3c3373eeb30',
        'a61fc56e-0d6f-4de4-8a48-f6084831d341',
        'af757ae5-1d90-4f6b-a778-72401619971e',
        'ff9fb400-15da-40a9-b8fc-991d1222f25a'
      ],
      information:
        '{"education":[{"school":"International Unviversity","major":"Information Technology","graduatedYear":"2024"},{"school":"BKU","major":"Nonono","graduatedYear":"2024"},{"school":"DH Quoc Te","major":"IT","graduatedYear":"2022"}],"experience":[{"companyName":"JobPort","position":"Team Leader","dateFrom":"2023-06-30","dateTo":"2023-08-04"}],"certificate":[{"name":"AWS Certification","id":"ix-130s-9djk","receivedDate":"2022-12-27","url":""}],"project":[],"skills":[{"label":"Python","value":"1"},{"label":"Java","value":"2"},{"label":"C#","value":"3"},{"label":"C++","value":"4"},{"label":"JavaScript","value":"5"},{"label":"ReactJs","value":"7"},{"label":"VueJs","value":"8"},{"label":"NodeJs","value":"6"}]}',
      blackList: false
    },
    {
      userId: 'f4ea3b52-7c83-47ff-bf75-ad525415a0c1',
      email: 'huutrong1101@gmail.com',
      phone: '0773696410',
      fullName: 'Nguyễn Hữu Trọng 1 ',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1696675552/Recruiment%20Assets/User/qqx98dv03bkyizcn60tj.jpg',
      address: 'Tp Hồ Chí Minh ',
      about: 'Backend Developer with 2+ years experiences.',
      skills: [],
      dateOfBirth: '2001-10-14T00:00:00.000+07:00',
      jobIds: [
        '1',
        '32a82915-0ecf-4352-a57d-9d962330a53a',
        '349e5b76-e9ac-4ae1-a352-b1523c470f0d',
        '423a4b6c-dd3b-4710-a3f1-5e37496d1d64',
        '5f3365ec-d703-4b8f-8569-84f224e0625c',
        '7c2a98ff-2f13-4eab-b858-6d98bbb0bd6f',
        '85315764-31e9-4688-a83c-6f733122700b',
        '8a010f3b-19b2-4188-92de-e3c3373eeb30',
        '8aa26130-69b6-40d1-93c7-2fc593976d66',
        'a61fc56e-0d6f-4de4-8a48-f6084831d341',
        'af757ae5-1d90-4f6b-a778-72401619971e',
        'c8710b00-3767-4e08-b53f-3ddded9c067c',
        'ff9fb400-15da-40a9-b8fc-991d1222f25a'
      ],
      information:
        '{"education":[{"school":"HCMUTE","major":"123","graduatedYear":"2024"}],"experience":[{"companyName":"FPT","position":"FE","dateFrom":"2023-02-20","dateTo":"2023-03-20"}],"certificate":[{"name":"FPT","id":"test","receivedDate":"2002-02-18","url":"test"}],"project":[{"name":"test","description":"test","url":"test"}]}',
      blackList: false
    },
    {
      userId: '707e1974-07dc-4bf1-bcff-4fc2ddfcfb4e',
      email: 'habangnguyen1234@gmail.com',
      phone: '0373412444',
      fullName: 'Nguyễn Ngọc Băng',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1691202243/Recruiment%20Assets/User/aw1dzztbsahww8aa51kt.jpg',
      address: 'Tp Hồ Chí Minh',
      about: 'Yêu thích lập trình, có 2 năm kinh nghiệm trong ngôn ngữ Java',
      skills: [],
      dateOfBirth: '2000-06-19T07:00:00.000+07:00',
      jobIds: ['55c399cd-0a78-469f-9032-90a8ba2ea66f'],
      information: '{"education":[],"experience":[],"certificate":[],"project":[]}',
      blackList: false
    }
  ],
  question: [
    {
      questionId: '311febcb-848c-4b3c-99a9-4d344437c038',
      content: 'tui là ai ',
      note: 'trịnh minh huy ',
      typeQuestion: 'TECHNICAL',
      skill: 'Golang',
      deleted: false
    },
    {
      questionId: '369b7dd5-c574-41ec-a5dc-fb7d2a58ac5f',
      content: 'minh pro',
      note: '',
      typeQuestion: 'TECHNICAL',
      skill: 'C#',
      deleted: false
    }
  ],
  candidateInteview: [
    {
      name: 'Hoàng Gia Bách',
      position: 'BACKEND',
      desc: 'Có khả năng quản lý thời gian hiệu quả và giải quyết vấn đề tốt',
      date: '2023-08-04T04:50:59.000+07:00',
      state: 'NOT_RECEIVED',
      score: -1.0,
      interviewId: '2899373d-208b-4bf4-a263-203308ae92d6',
      jobName: 'Lập Trình Viên Back-End (PHP Laravel, Nodejs)',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690452509/Recruiment%20Assets/User/p8x0pulbulhjkfodk37y.jpg',
      address: null,
      about: 'Có khả năng quản lý thời gian hiệu quả và giải quyết vấn đề tốt',
      jobSkills: [
        {
          skillId: '3',
          name: 'C#'
        },
        {
          skillId: '3',
          name: 'C#'
        }
      ],
      dateOfBirth: '2000-05-07T07:00:00.000+07:00',
      information: null,
      phone: null,
      email: 'nhnguyen.forwork@gmail.com',
      cv: null
    },
    {
      name: 'Nguyễn Hữu Trọng 1 ',
      position: 'FRONTEND',
      desc: 'Backend Developer with 2+ years experiences.',
      date: '2023-08-15T06:26:36.000+07:00',
      state: 'FAILED',
      score: 20.0,
      interviewId: '51c30f69-29d4-4599-924b-f33116301310',
      jobName: 'Fullstack Developer Clouds',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1696675552/Recruiment%20Assets/User/qqx98dv03bkyizcn60tj.jpg',
      address: 'Tp Hồ Chí Minh ',
      about: 'Backend Developer with 2+ years experiences.',
      jobSkills: [
        {
          skillId: '5',
          name: 'JavaScript'
        },
        {
          skillId: '6',
          name: 'NodeJs'
        },
        {
          skillId: '3',
          name: 'C#'
        }
      ],
      dateOfBirth: '2001-10-14T00:00:00.000+07:00',
      information:
        '{"education":[{"school":"HCMUTE","major":"123","graduatedYear":"2024"}],"experience":[{"companyName":"FPT","position":"FE","dateFrom":"2023-02-20","dateTo":"2023-03-20"}],"certificate":[{"name":"FPT","id":"test","receivedDate":"2002-02-18","url":"test"}],"project":[{"name":"test","description":"test","url":"test"}]}',
      phone: '0773696410',
      email: 'huutrong1101@gmail.com',
      cv: 'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690791695/Recruiment%20Assets/CV/CV.pdf.pdf'
    },
    {
      name: 'Nguyễn Huỳnh Nguyên',
      position: 'FRONTEND',
      desc: 'The Frontend Developer with 2+ years experiences.',
      date: '2023-08-07T17:35:38.129+07:00',
      state: 'RECEIVED',
      score: -1.0,
      interviewId: '0bb7c591-c708-46e9-8d3d-f47a684965db',
      jobName: 'Product & Service Trainer',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690861840/Recruiment%20Assets/User/d7n2n3sxpruvzyxmh2uy.jpg',
      address: 'Thủ Đức-TPHCM',
      about: 'The Frontend Developer with 2+ years experiences.',
      jobSkills: [
        {
          skillId: '7',
          name: 'ReactJs'
        },
        {
          skillId: '11',
          name: 'SQL '
        }
      ],
      dateOfBirth: '2002-03-27T07:00:00.000+07:00',
      information:
        '{"education":[{"school":"International Unviversity","major":"Information Technology","graduatedYear":"2024"},{"school":"BKU","major":"Nonono","graduatedYear":"2024"},{"school":"DH Quoc Te","major":"IT","graduatedYear":"2022"}],"experience":[{"companyName":"JobPort","position":"Team Leader","dateFrom":"2023-06-30","dateTo":"2023-08-04"}],"certificate":[{"name":"AWS Certification","id":"ix-130s-9djk","receivedDate":"2022-12-27","url":""}],"project":[],"skills":[{"label":"Python","value":"1"},{"label":"Java","value":"2"},{"label":"C#","value":"3"},{"label":"C++","value":"4"},{"label":"JavaScript","value":"5"},{"label":"ReactJs","value":"7"},{"label":"VueJs","value":"8"},{"label":"NodeJs","value":"6"}]}',
      phone: '0777752703',
      email: 'nhnguyen.forwork@gmail.com',
      cv: 'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690873255/Recruiment%20Assets/CV/internship_resume.pdf.pdf'
    },
    {
      name: 'Trọng Nguyễn Hữu',
      position: 'BACKEND',
      desc: 'Frontend Developer with 5+ years experiences.',
      date: '2023-08-08T10:44:17.000+07:00',
      state: 'RECEIVED',
      score: -1.0,
      interviewId: '59635486-4924-4440-a95e-f3bac708f7af',
      jobName: 'WEB SECURITY ',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1691292613/Recruiment%20Assets/User/xh1alikfohghxikvb5t0.jpg',
      address: 'HCM',
      about: 'Frontend Developer with 5+ years experiences.',
      jobSkills: [
        {
          skillId: '1',
          name: 'Python'
        }
      ],
      dateOfBirth: '2003-02-18T07:00:00.000+07:00',
      information:
        '{"education":[{"school":"HCMUTE","major":"CNTT","graduatedYear":"2024"},{"school":"HCMUT","major":"CNTT","graduatedYear":"2025"},{"school":"UEH","major":"CNTT","graduatedYear":"2025"},{"school":"UEF","major":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","graduatedYear":"2026"},{"school":"UIT","major":"CNTT","graduatedYear":"2027"},{"school":"","major":"","graduatedYear":""}],"experience":[{"companyName":"FPT","position":"FE","dateFrom":"2002-02-18","dateTo":"2023-03-19"}],"certificate":[{"name":"AWS","id":"123","receivedDate":"2002-02-18","url":"qssdqdq"}],"project":[],"skills":[{"value":"reactjs","label":"ReactJS"},{"value":"c and csharp","label":"C/C++"},{"value":"html","label":"HTML"}]}',
      phone: '0773696411',
      email: '20110754@student.hcmute.edu.vn',
      cv: 'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690791695/Recruiment%20Assets/CV/CV.pdf.pdf'
    },
    {
      name: 'Lê Thanh Khải',
      position: 'FRONTEND',
      desc: 'Mong muốn tìm được công việc ổn định và phù hợp với định hướng tương lai',
      date: '2023-08-07T06:55:02.589+07:00',
      state: 'RECEIVED',
      score: -1.0,
      interviewId: 'd9f3b5a5-a95f-4bc6-a88a-142e1c8ccf7e',
      jobName: 'Fullstack Developer Clouds',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1691084247/Recruiment%20Assets/User/rme2f3mn4ofaoky83pcj.png',
      address: null,
      about: 'Mong muốn tìm được công việc ổn định và phù hợp với định hướng tương lai',
      jobSkills: [
        {
          skillId: '5',
          name: 'JavaScript'
        },
        {
          skillId: '6',
          name: 'NodeJs'
        },
        {
          skillId: '3',
          name: 'C#'
        }
      ],
      dateOfBirth: '2002-04-12T00:00:00.000+07:00',
      information:
        '{"education":[{"school":"BK","major":"IT","graduatedYear":"2030"},{"school":"BK 2","major":"study again","graduatedYear":"2040"}],"experience":[{"companyName":"FPT","position":"Dev","dateFrom":"2023-06-12","dateTo":"2023-08-12"}],"certificate":[{"name":"FB user","id":"Khải","receivedDate":"2015-01-01","url":"https://www.facebook.com/khai.thanh.1204xx/"},{"name":"INS user","id":"id","receivedDate":"2020-01-01","url":""}],"project":[{"name":"Mock Project","description":"Recruitment","url":"http://localhost:5173/"},{"name":"Youtube","description":"Small Project","url":"https://www.youtube.com/watch?v=muAFXVFMUoA"}],"skills":[{"value":"reactjs","label":"ReactJS"},{"value":"java","label":"Java"},{"label":"VueJs","value":"8"},{"label":"C#","value":"3"}]}',
      phone: '0942594519',
      email: 'lethanhkhai12@gmail.com',
      cv: 'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690791695/Recruiment%20Assets/CV/CV.pdf.pdf'
    },
    {
      name: 'Bùi Đặng Quốc Chí ',
      position: 'FRONTEND',
      desc: 'Có khả năng diễn đạt ý kiến một cách rõ ràng và hiệu quả. Thông thao ngôn ngữ lập trình Java với 5 năm kinh nghiệm',
      date: '2023-08-07T17:35:44.213+07:00',
      state: 'RECEIVED',
      score: -1.0,
      interviewId: 'dfe7253c-b84b-4e2a-81a9-aad73de6154d',
      jobName: 'Product & Service Trainer',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1691377892/Recruiment%20Assets/User/x1cow3kctuykkqzxnugk.jpg',
      address: 'Thủ Đức',
      about:
        'Có khả năng diễn đạt ý kiến một cách rõ ràng và hiệu quả. Thông thao ngôn ngữ lập trình Java với 5 năm kinh nghiệm',
      jobSkills: [
        {
          skillId: '7',
          name: 'ReactJs'
        },
        {
          skillId: '11',
          name: 'SQL '
        }
      ],
      dateOfBirth: '2002-05-19T07:00:00.000+07:00',
      information:
        '{"education":[{"school":"HCMUTE","major":"CNTT","graduatedYear":"2020-2024"}],"experience":[{"companyName":"HCMUTE","position":"Student","dateFrom":"2020-10-10","dateTo":"2024-10-07"}],"certificate":[{"name":"AWS  RDS","id":"123","receivedDate":"2023-08-10","url":"https://www.facebook.com/quoc.chi.752487/"}],"project":[],"skills":[{"label":"Java","value":"2"},{"label":"C++","value":"4"},{"label":"JavaScript","value":"5"},{"label":"C#","value":"3"},{"label":"NodeJs","value":"6"},{"label":"ReactJs","value":"7"}]}',
      phone: '0362858058',
      email: 'quocchi1905@gmail.com',
      cv: 'https://res.cloudinary.com/dc6weg8vp/image/upload/v1691376698/Recruiment%20Assets/CV/CV_BuiDangQuocChi_0362858057.pdf.pdf'
    },
    {
      name: 'Nguyễn Hữu Trọng 1 ',
      position: 'FRONTEND',
      desc: 'Backend Developer with 2+ years experiences.',
      date: '2023-08-03T15:18:15.000+07:00',
      state: 'RECEIVED',
      score: -1.0,
      interviewId: '5ec4507a-caf9-4662-9d25-b1d1f14efd39',
      jobName: 'Lập trình viên Web Java',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1696675552/Recruiment%20Assets/User/qqx98dv03bkyizcn60tj.jpg',
      address: 'Tp Hồ Chí Minh ',
      about: 'Backend Developer with 2+ years experiences.',
      jobSkills: [
        {
          skillId: '1',
          name: 'Python'
        }
      ],
      dateOfBirth: '2001-10-14T00:00:00.000+07:00',
      information:
        '{"education":[{"school":"HCMUTE","major":"123","graduatedYear":"2024"}],"experience":[{"companyName":"FPT","position":"FE","dateFrom":"2023-02-20","dateTo":"2023-03-20"}],"certificate":[{"name":"FPT","id":"test","receivedDate":"2002-02-18","url":"test"}],"project":[{"name":"test","description":"test","url":"test"}]}',
      phone: '0773696410',
      email: 'huutrong1101@gmail.com',
      cv: 'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690720984/Recruiment%20Assets/CV/cv_707e1974-07dc-4bf1-bcff-4fc2ddfcfb4e.pdf'
    },
    {
      name: 'Nguyễn Hữu Trọng 1 ',
      position: 'BACKEND',
      desc: 'Backend Developer with 2+ years experiences.',
      date: '2023-08-12T06:05:38.000+07:00',
      state: 'NOT_RECEIVED',
      score: -1.0,
      interviewId: '79968b88-26e8-48d7-b1f2-de7132fd95e1',
      jobName: 'Junior Java Developer',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1696675552/Recruiment%20Assets/User/qqx98dv03bkyizcn60tj.jpg',
      address: 'Tp Hồ Chí Minh ',
      about: 'Backend Developer with 2+ years experiences.',
      jobSkills: [
        {
          skillId: '2',
          name: 'Java'
        }
      ],
      dateOfBirth: '2001-10-14T00:00:00.000+07:00',
      information:
        '{"education":[{"school":"HCMUTE","major":"123","graduatedYear":"2024"}],"experience":[{"companyName":"FPT","position":"FE","dateFrom":"2023-02-20","dateTo":"2023-03-20"}],"certificate":[{"name":"FPT","id":"test","receivedDate":"2002-02-18","url":"test"}],"project":[{"name":"test","description":"test","url":"test"}]}',
      phone: '0773696410',
      email: 'huutrong1101@gmail.com',
      cv: 'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690791695/Recruiment%20Assets/CV/CV.pdf.pdf'
    },
    {
      name: 'Nguyễn Huỳnh Nguyên',
      position: 'FRONTEND',
      desc: 'The Frontend Developer with 2+ years experiences.',
      date: '2023-08-08T23:14:41.000+07:00',
      state: 'FAILED',
      score: -1.0,
      interviewId: 'bc23254c-2c93-4735-a05a-906eb3da024c',
      jobName: 'Cloud Security Servive Engineer\r\n',
      avatar:
        'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690861840/Recruiment%20Assets/User/d7n2n3sxpruvzyxmh2uy.jpg',
      address: 'Thủ Đức-TPHCM',
      about: 'The Frontend Developer with 2+ years experiences.',
      jobSkills: [
        {
          skillId: '3',
          name: 'C#'
        },
        {
          skillId: '6',
          name: 'NodeJs'
        },
        {
          skillId: '7',
          name: 'ReactJs'
        },
        {
          skillId: '4',
          name: 'C++'
        }
      ],
      dateOfBirth: '2002-03-27T07:00:00.000+07:00',
      information:
        '{"education":[{"school":"International Unviversity","major":"Information Technology","graduatedYear":"2024"},{"school":"BKU","major":"Nonono","graduatedYear":"2024"},{"school":"DH Quoc Te","major":"IT","graduatedYear":"2022"}],"experience":[{"companyName":"JobPort","position":"Team Leader","dateFrom":"2023-06-30","dateTo":"2023-08-04"}],"certificate":[{"name":"AWS Certification","id":"ix-130s-9djk","receivedDate":"2022-12-27","url":""}],"project":[],"skills":[{"label":"Python","value":"1"},{"label":"Java","value":"2"},{"label":"C#","value":"3"},{"label":"C++","value":"4"},{"label":"JavaScript","value":"5"},{"label":"ReactJs","value":"7"},{"label":"VueJs","value":"8"},{"label":"NodeJs","value":"6"}]}',
      phone: '0777752703',
      email: 'nhnguyen.forwork@gmail.com',
      cv: 'https://res.cloudinary.com/dc6weg8vp/image/upload/v1690873255/Recruiment%20Assets/CV/internship_resume.pdf.pdf'
    },
    {
      name: 'An Ngo ne ne',
      position: 'FRONTEND',
      desc: null,
      date: '2023-08-08T10:00:59.000+07:00',
      state: 'PASSED',
      score: 90.0,
      interviewId: '71c98fd6-a11a-451f-ad6d-38894372cb14',
      jobName: 'Python Developer',
      avatar: null,
      address: null,
      about: null,
      jobSkills: [
        {
          skillId: '1',
          name: 'Python'
        },
        {
          skillId: '4',
          name: 'C++'
        }
      ],
      dateOfBirth: null,
      information:
        '{"education":[],"experience":[{"companyName":"FPT","position":"BE","dateFrom":"2023-08-11","dateTo":"2023-08-27"}],"certificate":[],"project":[],"skills":[{"label":"Python","value":"1"},{"label":"AWS","value":"15"},{"label":"SQL ","value":"11"}]}',
      phone: '0394595118',
      email: 'thanh20080kg@gmail.com',
      cv: 'https://res.cloudinary.com/dc6weg8vp/image/upload/v1691462493/Recruiment%20Assets/CV/Nguyen-Duc-Thinh-TopCV.vn-261222.115649.pdf.pdf'
    }
  ],
  interview: [
    {
      interviewId: '8e01688c-0762-435e-aced-0b8ef5007a7c',
      jobApplyState: 'RECEIVED',
      jobName: 'Chuyên viên Content SEO',
      time: '2023-08-25T22:07:19.000+07:00',
      interviewerFullNames: [
        'Nguyễn Huy Khiêm',
        'Phạm Khánh Vân',
        'Lê Vũ Trường Giang',
        'Đặng Nam Hải Triều',
        'Phan Minh Hiếu',
        'Hồ Ngọc Hà'
      ],
      interviewLink:
        'https://teams.microsoft.com/l/meetup-join/19%3ameeting_MjAxYzgxMGEtZWYxOS00MDM5LWEzZGQtY2M0NDhhOTMzZmNm%40thread.v2/0?context=%7b%22Tid%22%3a%2228a837bf-14c0-41ae-9387-fe0082be35b4%22%2c%22Oid%22%3a%226062f1a6-e95a-404b-bbd2-022412d96ff9%22%7d',
      position: 'BACKEND'
    },
    {
      interviewId: 'd7b03efd-4727-4096-a9df-e6219b62b148',
      jobApplyState: 'RECEIVED',
      jobName: 'Automation Test',
      time: '2023-08-17T07:40:22.000+07:00',
      interviewerFullNames: ['Nguyễn Huy Khiêm'],
      interviewLink:
        'https://teams.microsoft.com/l/meetup-join/19%3ameeting_ZjFkYTU4YjEtODVmYi00ZGM5LWJhZmItNjAxYzBiNjE5N2I0%40thread.v2/0?context=%7b%22Tid%22%3a%2228a837bf-14c0-41ae-9387-fe0082be35b4%22%2c%22Oid%22%3a%226062f1a6-e95a-404b-bbd2-022412d96ff9%22%7d',
      position: 'FRONTEND'
    },
    {
      interviewId: '51c30f69-29d4-4599-924b-f33116301310',
      jobApplyState: 'FAILED',
      jobName: 'Fullstack Developer Clouds',
      time: '2023-08-15T06:26:36.000+07:00',
      interviewerFullNames: [
        'Nguyễn Thị Hương',
        'Trần Văn Kiên',
        'Nguyễn Huy Khiêm',
        'Nguyễn Hải Anh',
        'Lê Vũ Trường Giang',
        'Nguyễn Văn Lương',
        'Nguyễn Văn Bá'
      ],
      interviewLink:
        'https://teams.microsoft.com/l/meetup-join/19%3ameeting_NTc1N2UwNWItMjA3OS00NGFmLWFmZTItMDVkMjM1OTY2NmVj%40thread.v2/0?context=%7b%22Tid%22%3a%2228a837bf-14c0-41ae-9387-fe0082be35b4%22%2c%22Oid%22%3a%226062f1a6-e95a-404b-bbd2-022412d96ff9%22%7d',
      position: 'FRONTEND'
    },
    {
      interviewId: '79968b88-26e8-48d7-b1f2-de7132fd95e1',
      jobApplyState: 'NOT_RECEIVED',
      jobName: 'Junior Java Developer',
      time: '2023-08-12T06:05:38.000+07:00',
      interviewerFullNames: ['Nguyễn Huy Khiêm', 'Phạm Khánh Vân'],
      interviewLink:
        'https://teams.microsoft.com/l/meetup-join/19%3ameeting_Nzk0MDFmNDYtMDIwNS00YzU1LTg0MTItNTFiOTBkNjFlMjQy%40thread.v2/0?context=%7b%22Tid%22%3a%2228a837bf-14c0-41ae-9387-fe0082be35b4%22%2c%22Oid%22%3a%226062f1a6-e95a-404b-bbd2-022412d96ff9%22%7d',
      position: 'BACKEND'
    },
    {
      interviewId: '3775b093-9447-4fc8-9234-7eb949c11a4f',
      jobApplyState: 'NOT_RECEIVED',
      jobName: 'Java Fullstack Developer',
      time: '2023-08-09T18:00:13.000+07:00',
      interviewerFullNames: ['Nguyễn Huy Khiêm', 'Phạm Khánh Vân'],
      interviewLink:
        'https://teams.microsoft.com/l/meetup-join/19%3ameeting_M2JmYmE2OWMtNjY1MS00MDVkLTgzN2MtYjEyYTYxYzM0NmY3%40thread.v2/0?context=%7b%22Tid%22%3a%2228a837bf-14c0-41ae-9387-fe0082be35b4%22%2c%22Oid%22%3a%226062f1a6-e95a-404b-bbd2-022412d96ff9%22%7d',
      position: 'FRONTEND'
    },
    {
      interviewId: 'bc23254c-2c93-4735-a05a-906eb3da024c',
      jobApplyState: 'FAILED',
      jobName: 'Cloud Security Servive Engineer\r\n',
      time: '2023-08-08T23:14:41.000+07:00',
      interviewerFullNames: ['Nguyễn Huy Khiêm', 'Phạm Khánh Vân', 'Đặng Nam Hải Triều', 'Hồ Ngọc Hà'],
      interviewLink:
        'https://teams.microsoft.com/l/meetup-join/19%3ameeting_YWNlNWJhNzUtMTNmNC00M2I2LWI1Y2QtZDJkYTlhYjViMjAw%40thread.v2/0?context=%7b%22Tid%22%3a%2228a837bf-14c0-41ae-9387-fe0082be35b4%22%2c%22Oid%22%3a%226062f1a6-e95a-404b-bbd2-022412d96ff9%22%7d',
      position: 'FRONTEND'
    },
    {
      interviewId: '57be3e8e-919f-41b8-988b-a9165440230e',
      jobApplyState: 'RECEIVED',
      jobName: 'Fresher ReactJS Developer ',
      time: '2023-08-08T19:10:01.785+07:00',
      interviewerFullNames: ['Nguyễn Huy Khiêm'],
      interviewLink:
        'https://teams.microsoft.com/l/meetup-join/19%3ameeting_MjA4OWUyZTctYTk4Yi00ZjI3LWE1YTktYzJkNjk4NTZjNTFl%40thread.v2/0?context=%7b%22Tid%22%3a%2228a837bf-14c0-41ae-9387-fe0082be35b4%22%2c%22Oid%22%3a%226062f1a6-e95a-404b-bbd2-022412d96ff9%22%7d',
      position: 'BACKEND'
    },
    {
      interviewId: '59635486-4924-4440-a95e-f3bac708f7af',
      jobApplyState: 'RECEIVED',
      jobName: 'WEB SECURITY ',
      time: '2023-08-08T10:44:17.000+07:00',
      interviewerFullNames: ['Nguyễn Thị Hương', 'Trần Văn Kiên', 'Nguyễn Huy Khiêm'],
      interviewLink:
        'https://teams.microsoft.com/l/meetup-join/19%3ameeting_ZDExZGI5NGItZWEwMS00Mjc2LWI5ZmUtNjBmZjU5NTc4ODg0%40thread.v2/0?context=%7b%22Tid%22%3a%2228a837bf-14c0-41ae-9387-fe0082be35b4%22%2c%22Oid%22%3a%226062f1a6-e95a-404b-bbd2-022412d96ff9%22%7d',
      position: 'BACKEND'
    },
    {
      interviewId: '71c98fd6-a11a-451f-ad6d-38894372cb14',
      jobApplyState: 'PASSED',
      jobName: 'Python Developer',
      time: '2023-08-08T10:00:59.000+07:00',
      interviewerFullNames: ['Trần Văn Kiên', 'Nguyễn Huy Khiêm', 'Lê Vũ Trường Giang'],
      interviewLink:
        'https://teams.microsoft.com/l/meetup-join/19%3ameeting_NDM2ZWQ0YTEtYWVlZS00MWY1LTljMzUtZWU3N2NlMjkxNTcy%40thread.v2/0?context=%7b%22Tid%22%3a%2228a837bf-14c0-41ae-9387-fe0082be35b4%22%2c%22Oid%22%3a%226062f1a6-e95a-404b-bbd2-022412d96ff9%22%7d',
      position: 'FRONTEND'
    },
    {
      interviewId: 'dfe7253c-b84b-4e2a-81a9-aad73de6154d',
      jobApplyState: 'RECEIVED',
      jobName: 'Product & Service Trainer',
      time: '2023-08-07T17:35:44.213+07:00',
      interviewerFullNames: ['Nguyễn Huy Khiêm'],
      interviewLink:
        'https://teams.microsoft.com/l/meetup-join/19%3ameeting_NWEzZDdlMTUtNWQyYy00YWZjLThlZDMtNjMyZmI3YTg1ZGE0%40thread.v2/0?context=%7b%22Tid%22%3a%2228a837bf-14c0-41ae-9387-fe0082be35b4%22%2c%22Oid%22%3a%226062f1a6-e95a-404b-bbd2-022412d96ff9%22%7d',
      position: 'FRONTEND'
    }
  ]
}
