...
function ResumeTemp1({ values, previewAvatar }: ResumeThemeProps) {
  ...
  return (
    <div className='w-full border border-zinc-100 min-h-[600px] px-8 py-6 relative'>
      <div className='flex items-center justify-between mb-5'>
        <div className='relative w-[70%] pb-12 pt-7 box-name text-[#007ea7]'>
          <div className='flex flex-col gap-2'>
            <div className='flex flex-col'>
              <h3 className='relative text-2xl font-bold'>{values.name ? values.name : 'HỌ VÀ TÊN'}</h3>
              <p className='text-xl'>{values.title ? values.title : 'TIÊU ĐỀ HỒ SƠ'}</p>
            </div>
            {values.goal && (
        ...
        <div>
          {previewAvatar ? (
            <img
              className='object-cover w-[150px] h-[150px] rounded-full border-2 border-blue-200'
              src={previewAvatar}
              alt={`Preview Avatar`}
            />
          ) : (
      ...
      <div className='flex items-start gap-3'>
        <div className='flex flex-col w-1/3 gap-2'>
          <div className='flex flex-col w-full'>
            <p className='text-sm font-bold text-center'>THÔNG TIN CƠ BẢN</p>
          ...
          {values.certifications && values.certifications.length > 0 && (
            <div className='flex flex-col w-full mt-2'>
              <p className='text-sm font-bold text-center'>CHỨNG CHỈ</p>
          ...
          {shouldShowAdditionalInformation(values) && (
            <div className='flex flex-col w-full mt-2'>
              <p className='text-sm font-bold text-center'>THÔNG TIN BỔ SUNG</p>
        ...
        <div className='flex flex-col w-2/3 gap-2'>
          {values.workHistories && values.workHistories.length > 0 && (
            <div className='flex flex-col w-full'>
              <p className='text-sm font-bold text-center'>KINH NGHIỆM LÀM VIỆC</p>
          ...
          {values.educations && values.educations.length > 0 && (
            <div className='flex flex-col w-full'>
              <p className='text-sm font-bold text-center'>HỌC VẤN</p>
          ...
          {values.activity && (
            <div className='flex flex-col w-full mt-2'>
              <p className='text-sm font-bold text-center uppercase'>Giải thưởng / Hoạt động ngoại khoá</p>
