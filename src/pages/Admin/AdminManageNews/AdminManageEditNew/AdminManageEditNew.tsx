...
function AdminManageEditNew() {
  const dispatch = useAppDispatch()
  const navigate = useNavigate()

  const { newId } = useParams()

  const newDetail = useAppSelector((state) => state.AdminSlice.newDetail)
  ...
  useEffect(() => {
    if (newId) {
      AdminService.getNewDetail(dispatch, newId)
    }
  }, [dispatch, newId])

  useEffect(() => {
    if (newDetail) {
      const thumbnail: UploadFile | undefined = newDetail.thumbnail
        ? {
            uid: '-logo',
            name: 'logo.png',
            status: 'done',
            url: newDetail.thumbnail
          }
        : undefined
  ...
  const onFinish = () => {
    ...
    if (newDetail) {
      toast
        .promise(AdminService.updateNewInformation(formDataObj, newDetail._id), {
          pending: `Tin tức đang được cập nhật`,
  ...
  return (
    <div className='flex flex-col flex-1 gap-4'>
      <div className='w-full p-4'>
        <div className='mb-2'>
          <h1 className='flex-1 text-2xl font-semibold text-center md:mb-4'>Chỉnh sửa tin tức</h1>
        </div>
...
export default AdminManageEditNew
