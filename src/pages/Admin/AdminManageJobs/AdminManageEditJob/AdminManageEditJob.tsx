...
function AdminManageEditJob() {
  ...
  const [initialCompany, setInitialCompany] = useState<ActivityOption>()
  ...
  useEffect(() => {
    if (jobDetail && form) {
      ...
      const matchingCompany = optionsRec.find((option) => option.label === jobDetail.companyName)

      form.setFieldsValue({
        recruiterId: matchingCompany?.value,
        name: jobDetail.name,
  ...
  const onFinish = () => {
    ...
    if (jobDetail) {
      toast
        .promise(AdminService.updateJobInformation(formValues, jobDetail?._id), {
          pending: `Công việc đang được cập nhật`,
  ...
  return (
    <div className='flex flex-col flex-1 gap-4'>
      <div className='w-full p-4'>
        <div className='mb-2'>
          <h1 className='flex-1 text-2xl font-semibold text-center md:mb-4'>Chỉnh sửa tin tuyển dụng</h1>
        </div>
            ...
            <div className='flex items-center justify-center gap-2'>
              ...
              >
                <DatePicker
                  ...
                  format='DD/MM/YYYY' // Đặt lại định dạng hiển thị cho DatePicker
...
export default AdminManageEditJob
