...
const InterviewDetail = () => {
  ...
  const handleChange = (event: any) => {
    ...
    params.append('content', textFilter)
    params.append('skill', skill)
    params.append('type', type)

    dispatch(fetchINTQuestionData(params))
  ...
  const showCandidate = () => {
    dispatch(fetchINTCandidatesByID(INTSingleInterview.candidate.candidateId))
    setView(true)
  }
      ...
      <div className='px-6 py-6 mt-8 border-2 shadow-xl rounded-xl'>
        ...
        <div className='text-base'>
          ...
          <span className='ml-2 text-sm'>
            {formatHHMM(INTSingleInterview?.Date) + ' ' + formatDDMMYY(INTSingleInterview?.Date)}
          </span>
        </div>
        <div className='text-base'>
          Link Meeting:
            ...
            <div>
              <table className='w-full border border-collapse border-gray-300 rounded table-auto'>
                ...
                <tbody>
                  ...
                  {assignedQuestionsStatus === STATUS.IDLE && assignedQuestions.length > 0
                    ? assignedQuestions?.map((item: any) => (
                        <tr key={item.questionId} className='bg-white'>
                          <td className='px-4 py-2 border'>{item.content}</td>
